...
class CallableEmpyricalProxy(object):
...
class EmpyricalArrayProxy(object):
    def __init__(self, explicit=False, pandas_only=False):
        self._explicit = explicit
        self._pandas_only = pandas_only

    @classmethod
    def __call__(cls, explicit=False, pandas_only=False):
        return cls(explicit, pandas_only)
    ...
    def __getattr__(self, item):
        ...
        def call_empyrical_with_arrays(*args, **kwargs):
            ...
            return getattr(empyrical, item)(*args, **kwargs)

        call_empyrical_with_arrays.__name__ = item
        return call_empyrical_with_arrays


class EmpyricalProperty(object):
...
class TestStats(TestCase):
    ...
    def test_information_ratio_trans(self, returns, add_noise, translation):
        ir = self.empyrical.information_ratio(returns+add_noise, returns)
    ...
    def test_cagr_with_nan_inputs(self, returns):
        self.assertNotEqual(self.empyrical.cagr(returns), np.nan)  # FIXME??
